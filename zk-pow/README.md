...
The block header fields committed to before mining begins (version, prev\_block, merkle\_root, timestamp, nbits). The miner fills in the proof fields to complete the header.
...
### PlainProof → ZKProof pipeline
1. Miner performs `(A + E)·(B + F)` MatMul using committed noise matrices
2. A `PlainProof` is produced containing the Merkle-opened matrix rows/columns
3. `PlainProof` is converted to a compact `ZKProof` (≤ 60 KB) via Plonky2 recursive SNARKs
4. `ZKProof` is embedded in the block certificate and verified by all nodes

### Circuit Cache
Plonky2 circuits are expensive to compile (tens of seconds). The `build_cache` binary pre-compiles 13 first-level and 2 second-level circuits and serialises them to `src/circuit/cache.bin`. At runtime the verifier loads this cache for instant verification.
...
# Build with embedded circuit cache (no external file needed)
...
Go FFI
The bindings/go/ directory contains a cbindgen-generated C header and Go wrapper. Build the shared library with:
